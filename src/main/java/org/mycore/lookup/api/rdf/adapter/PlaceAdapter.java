 ...
 *
 */
public class PlaceAdapter extends RDFMappingAdapter<String, Place> {

    /* (non-Javadoc)
     * @see org.mycore.lookup.api.rdf.adapter.RDFMappingAdapter#unmarshal(java.lang.Object)
     */
    @Override
    public Place unmarshal(String v) {
        if (v != null) {
            try {
                URI uri = new URI(v);
                Place p = RDFMapper.map(uri, Place.class);
                if (p == null) {
                    p = new Place();
                    p.setName(v);
                } else {
