 ...
 *
 */
public class SameAsAdapter extends RDFMappingAdapter<List<String>, List<IdType>> {

    /* (non-Javadoc)
     * @see org.mycore.lookup.api.rdf.adapter.RDFMappingAdapter#unmarshal(java.lang.Object)
     */
    @Override
    public List<IdType> unmarshal(List<String> v) {
        return v.stream().map(s -> {
            try {
                return Scheme.extractIdTypeFromURI(new URI(s));
     ...
     */
    @Override
    public List<String> marshal(List<IdType> v) {
        return v.stream().map(it -> it.authorityURI().toString()).filter(u -> u != null).collect(Collectors.toList());
    }
